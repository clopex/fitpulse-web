...
import { getClassesApi, bookClassApi, getMyBookings } from '@/api/classes.api';
...
export default function ClassesPage() {
  ...
  const bookedClassIds = useMemo(() => {
    ...
    return new Set(
      bookings
        ...
        .map((booking: any) => booking.class_id)
    );
  }, [bookingsData]);
  ...
  const handleBook = async (classId: string) => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ['classes-home'] });
    } catch (err: any) {
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {classes.map((c: any) => {
            const isBooked = bookedClassIds.has(c.id) || bookingStates[c.id] === 'booked';
            ...
            return (
              ...
              {c.is_cancelled ? (
                ...
                <button
                  disabled
                  className="w-full py-2.5 rounded-lg text-sm font-medium border border-border text-muted-foreground disabled:opacity-50 mt-auto"
                >
                  Loading...
                </button>
              ) : isAuthenticated ? (
                <button
                  ...
                  disabled={bookingStates[c.id] === 'loading' || isBooked || c.booked_count >= c.capacity}
